...
//      )})
  ...
  fun search(categoryID: String? = null, query: String? = null) {
    viewModelScope.launch {
      images.clear()
      images.addAll(0, apiService.searchImages(categoryID).data.map {
        Image(
          it.id,
          it.assets["preview"]!!.url,
          it.description,
